...
public class GameLobby {
    ...
    public void enableDLC() {
        List<Town> dlcTownList = dataProvider.getDLCTownsList();
        Set<Hero> dlcHeroSet = dataProvider.getDLCHeroesSet();
        playableTownsWithHeroesList.putAll(mapHeroesToStartingTowns(dlcTownList, dlcHeroSet));
    }
    ...
    public void disableDLC() {
        List<Town> dlcTownList = dataProvider.getDLCTownsList();
        dlcTownList.forEach(playableTownsWithHeroesList.keySet()::remove);
    }
    ...
    public Map<Town, List<Hero>> mapHeroesToStartingTowns(List<Town> availableTowns, Set<Hero> availableHeroes) {
        ...
        for(Town town : availableTowns){
            ...
            townHeroMap.put(town, heroesInTown);
        }
        return townHeroMap;
    }
    ...
    public Hero selectHeroByName(Town heroTown, String name) {
        List<Hero> heroesInTown = playableTownsWithHeroesList.get(heroTown);
        if(heroesInTown != null){
            for(Hero hero : heroesInTown){
                if (hero.getName().equalsIgnoreCase(name)) {
                    heroesInTown.remove(hero);
                    return hero;
                }
            }
        }
